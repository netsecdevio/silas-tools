...
test('sessions command uses shared inline resolver in all node scripts', () => {
  ...
  assert.strictEqual((sessionsDoc.match(/\['marketplace','silas-tools'\]/g) || []).length, 6);
  assert.strictEqual((sessionsDoc.match(/\['ecc','silas-tools'\]/g) || []).length, 6);
});
...
test('skill-health command uses shared inline resolver in all shell snippets', () => {
  ...
  assert.strictEqual((skillHealthDoc.match(/\['marketplace','silas-tools'\]/g) || []).length, 3);
  assert.strictEqual((skillHealthDoc.match(/\['ecc','silas-tools'\]/g) || []).length, 3);
});

test('inline resolver covers current and legacy marketplace plugin roots', () => {
  assert.ok(INLINE_RESOLVE.includes('"marketplace","ecc"'));
  assert.ok(INLINE_RESOLVE.includes('"marketplace","silas-tools"'));
});
