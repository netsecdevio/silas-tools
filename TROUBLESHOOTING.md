...
Common issues and solutions for silas-tools plugin.
...
**Solutions:**
```bash
# First inspect what silas-tools still knows about this machine
ecc list-installed
ecc doctor
ecc repair
...
# Claude Code → Extensions → silas-tools → Uninstall
# Then reinstall from marketplace

# If the issue is marketplace/account access, use silas-tools billing/account recovery separately; do not use reinstall as a proxy for account recovery
...
git clone https://github.com/netsecdevio/silas-tools.git
cp -r silas-tools ~/.claude/plugins/ecc
```
...
1. **Check GitHub Issues**: [github.com/netsecdevio/silas-tools/issues](https://github.com/netsecdevio/silas-tools/issues)
