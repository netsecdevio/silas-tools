...
**silas-tools's built-in persistent loop.** A session-aware REPL that calls `claude -p` synchronously with full conversation history.
...
3. **Any loop + Verification** — Use silas-tools's `/verify` command or `verification-loop` skill as a gate before commits.
...
| NanoClaw | silas-tools | `/claw` command in this repo |
| Verification Loop | silas-tools | `skills/verification-loop/` in this repo |
