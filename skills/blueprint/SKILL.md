...
This skill ships with silas-tools. No separate installation is needed when silas-tools is installed.

### Full silas-tools install

If you are working from the silas-tools repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the silas-tools diff before updating:

```bash
cd /path/to/silas-tools
...
If you are vendoring only this skill outside the full silas-tools install, copy the reviewed file from the silas-tools repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed silas-tools commit rather than running `git pull`.
