...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value silas-tools-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
origin: silas-tools
---
...
This is the silas-tools-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against silas-tools-native coverage
...
- Prefer silas-tools-native workflows over generic "install another plugin" advice when silas-tools can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in silas-tools
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent silas-tools workspaces
...
- existing silas-tools skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but silas-tools lacks a billing-operator skill"
- "Google Drive is connected, but there is no silas-tools-native Google Workspace operator workflow"
...
2. whether silas-tools already has parity
3. whether silas-tools only has primitives
4. whether silas-tools is missing the workflow entirely

### Phase 3: Turn Gaps Into silas-tools Decisions

For every real gap, recommend the correct silas-tools-native shape:

| Gap Type | Preferred silas-tools Shape |
|----------|---------------------|
...
2. **Parity**
   - where silas-tools already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but silas-tools lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete silas-tools-native additions, ordered by impact
...
- If silas-tools already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what silas-tools should own next.
